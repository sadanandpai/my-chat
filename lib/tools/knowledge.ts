...
export const searchKnowledgeTool = tool(
  ...
  },
  {
    ...
    description:
      "Look up DEEP details about Sadanand Pai's career story, skills, and technical work: interview rounds/format, day-to-day work at an employer, accomplishments, opinions shaped by his work, what he is focusing on now, career direction. ALWAYS call this for 'where do you see yourself in 5 years', career goals, what's next, or interview-style future questions — then only speak to directions present in the fragments (do not invent a title or a 5-year plan). ALWAYS call this when a company is named alongside anything beyond the bare fact of employment — rounds, teams, products built, how he left. First confirm the company via lookup_company when the ask is 'did you work/interview at X' or when listing employers; this tool is for depth after that, not the authoritative yes/no roster. Current title/seniority → also getIntro. Pure bio (location, contact/socials, hobbies, games) → getIntro. Employer list / 'where did you work' / bare 'did you work at X' → lookup_company. People → lookup_person. OSS project catalogs → lookup_projects (this tool is fine as follow-up for career-story context around how an OSS project started).",
    schema: z.object({
      query: z
        .string()
        .describe(
